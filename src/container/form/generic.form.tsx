                                                    ...
                                                    <ContainerInput2 error={validation(key).length !== 0 ? true : false}>
                                                        {Array.isArray(atribute[index]?.worth) || atribute[index]?.type === 'object' || atribute[index]?.type === 'undefined' ?
                                                            <select key={key} name={key} onChange={Array.isArray(value) ? handleInputChangeSubSelectArray : handleInputChangeSubSelect}
                                                                ...
                                                                <option selected key={Math.random()} value={value === undefined || value === null || value[0] === undefined ? null : Array.isArray(value) ? value[0] : value}>{value === undefined || value === null || value[0] === undefined ? null : Array.isArray(value) ? (value[0].hasOwnProperty('name') ? value[0]?.name : value[0]?.id) : value.name !== undefined ? value?.name : value?.id}</option>
