...
export const UserForm = () => {
    ...
    const createItem = async () => {
        await create("user_entity".toLowerCase(), state).then((data) => {
            validItem(data)
        }).catch((error) => { networkError() })
    }
    const retrieveItem = async () => {
        await retrieve("user_entity".toLowerCase(), page, size, "name").then((data) => {
            startTransition(() => setPageable(data) )
            startTransition(() => setStates(data.content) )
        }).catch((error) => { networkError() })
    }
    const updateItem = async () => {
        await update("user_entity".toLowerCase(), state).then((data) => {
            validItem(data)
        }).catch((error) => { networkError() })
    }
    const deleteItem = async () => {
        await remove("user_entity".toLowerCase(), state.id).then((data) => {
            validItem(data)
        }).catch((error) => { networkError() })
    }
    // const deleteAllItem = async () => {
    //     await removeAll("user_entity".toLowerCase()).then((data) => 
    //         handleModal()
    //     )
    // }
            ...
            {isValidToken() &&
                <Table>
                    ...
                    <ErrorBoundary fallback={<div> Algo deu errado </div>} >
                        <tbody>
                            {states.map((element) => {
                                return <tr onClick={() => selectItem(element)}><td>{element.id}</td><td>{element.username}</td></tr>
                            })}
                        </tbody>
                    </ErrorBoundary>
                    <tfoot>
